...
public class Utils {
    ...
    public static final String EVERY_1_HOURS = "every 1 hours";
    public static final String EVERY_DAY_00_00 = "every day 00:00";
    ...
    private static Map<String, Integer> buildCronModel() {
        ...
        result.put(EVERY_DAY_00_00, 1440);
        result.put(EVERY_1_HOURS, 60);
        result.put(EVERY_30_MINUTES, 30);
        result.put(EVERY_15_MINUTES, 15);
        result.put(EVERY_5_MINUTES, 5);
        return result;
    }
    ...
    private static String formatMinutesToWords(long numberOfMinutes, TimeGranularity timeGranularity) {
        ...
        if (timeGranularity == TimeGranularity.MAJOR_UNIT) {
            if (years != 0) {
                ...
                double decimals = 1.0 * (numberOfMinutesCopy - hours * numberOfMinutesInHour) / numberOfMinutesInHour;
                pairs.add(new Pair(hours + Math.round(decimals), " hour"));
            } else if (minutes != 0) {
        ...
        for (Pair pair : pairs) {
            if (pair.number > 0 || (pair.name.endsWith("minute") && sb.length() == 0)) {
                ...
                if (pair.decimals > 0) {
                    String decimals = String.format("%.1f", pair.decimals);
                    sb.append(decimals.substring(1));
                }
                sb.append(pair.name);
    ...
    public static Long[] createJobContext(Job job) {
        return createJobContext(job.getKey());
    }

    public static Long[] createJobContext(Key jobKey) {
        return new Long[]
                        {
                            jobKey.getId()
                        };
    ...
    private static Object getExportFilenameWithoutExtension(Job job) {
        return String.format("ping-service-%s", 
    ...
    private static String formatMillisecondsToWords(long totalTimeMillis, TimeGranularity timeGranularity) {
        ...
        if (totalTimeMillis <= 0) {
            totalTimeFormatted = "A moment";
        } else if (totalTimeMillis < getNumberOfMilliseconds(timeGranularity)) {
    ...
    public static boolean isCause(Throwable exception, Class<?> possibleCause) {
        return isCause(exception, possibleCause, null);
    }
    
    private static boolean isCause(Throwable exception, Class<?> possibleCause, Throwable topException) {
        if (exception == null) return false;
        if (exception == topException) return false;
        if (possibleCause.isAssignableFrom(exception.getClass())) return true;
        
        if (exception.getCause() == null) return false;
        if (possibleCause.isAssignableFrom(exception.getCause().getClass())) return true;
        
        return isCause(exception.getCause(), possibleCause, exception);
    }
